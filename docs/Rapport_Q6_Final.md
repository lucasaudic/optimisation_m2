...
### Protocol
*   **Machine** : Windows, Python (Spyder Runtime).
*   **Instances** : De 17 à 1379 villes.
*   **Algorithmes** : Exact (pour n<=20), Constructive (NN), Local Search (2-Opt après NN), GRASP (alpha=0.2, 50 itérations).

### Résultats Comparatifs

Le tableau de résultats complet est disponible dans `report/benchmark_q6_results.csv`.

#### Complexité Temporelle vs Taille

*(Insérer ici `report/plot_time_complexity.png` après génération)*

**Analyse** :
*   L'algorithme **Exact** explose en temps dès n > 20.
*   **Constructive** est extrêmement rapide, quasi instantané même pour les grandes instances, confirmant sa complexité O(n²).
*   **Local Search** est plus lent, mais reste praticable. Son temps augmente plus vite que le constructif.
*   **GRASP** est le plus lent des méthodes approchées car il répète la recherche locale 50 fois. C'est le prix à payer pour la qualité.

#### Qualité des Solutions

*(Insérer ici `report/plot_quality.png` après génération)*

**Analyse** :
*   **Constructive** fournit les solutions les moins bonnes.
*   **Local Search** améliore significativement la solution initiale.
*   **GRASP** fournit généralement les meilleures solutions parmi les méthodes approchées, grâce à la diversification des points de départ.

#### Écart à l'Optimal (Gap)

*(Insérer ici `report/plot_gap.png` après génération)*

**Analyse** :
*   Sur les petites instances, on observe que GRASP trouve souvent l'optimal ou en est très proche (< 1% gap).
*   Constructive peut avoir un gap important (> 5-10%).

---

### Conclusion
GRASP offre un excellent compromis pour ce problème. Bien que plus lent qu'une simple recherche locale, il permet d'échapper aux optima locaux et de trouver des solutions robustes, souvent proches de l'optimal pour les instances de taille modeste.
