...
\author{Lucas AUDIC, Axel BATTEUX, Romain PERIDY, Tristan DELEPINE \\ Master 2 Optimisation}
...
\subsection{Format d'Entrée et Génération des Instances}

Les instances utilisées dans ce projet sont basées sur la bibliothèque de référence \textbf{TSPLIB}, mais fournies sous un format pré-calculé. 

Les fichiers \texttt{.in} contiennent directement la **matrice complète des distances** (symétrique ou non), et non les coordonnées géographiques des villes. 
\begin{itemize}
    \item Cela signifie que les distances sont fixées par le fichier d'entrée et ne dépendent pas d'une formule de calcul (Euclidienne, Géographique, etc.).
    \item Les résultats obtenus (coûts) sont donc spécifiques à ces matrices de distances fournies et peuvent différer légèrement des optimums TSPLIB standard calculés à partir de coordonnées avec arrondi spécifique (NINT).
\end{itemize}
...
\subsection{Analyse de Stabilité (Grandes Instances)}

La stabilité est un critère crucial pour les méta-heuristiques stochastiques comme GRASP. Contrairement aux algorithmes déterministes (Nearest Neighbor, 2-Opt désidératif), GRASP peut produire des résultats différents à chaque exécution.

Pour évaluer cette stabilité, nous avons exécuté GRASP 10 fois sur l'instance \texttt{100.in} :
...
\hline
\end{tabular}
\caption{Analyse de stabilité de GRASP sur 10 exécutions (100 villes)}
\end{table}

\textbf{Conclusion} : Avec un coefficient de variation inférieur à 1\%, GRASP se montre \textbf{très stable}. La stratégie de diversification contrôlée par la RCL ($\alpha=0.2$) permet d'explorer efficacement sans comporter de risque excessif de mauvaise solution.
